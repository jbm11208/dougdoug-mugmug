...
public class MugMugEntity extends AbstractHorse {

    public MugMugEntity(EntityType<? extends AbstractHorse> type, Level world) {
        super(type, world);
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!held.isEmpty()) {
            if (this.isFood(held)) {
                ...
                if (this.isBaby()) {
                    this.usePlayerItem(player, hand, held);
                    this.ageUp(MugMugEntity.getSpeedUpSecondsWhenFeeding(-i), true);
                    return InteractionResult.SUCCESS;
                }
        ...
        if (!this.isTamed()) {
            if (held.is(Items.SUSPICIOUS_STEW)) {
                if (!this.level().isClientSide()) {
                    held.consume(1, player);
    ...
    public AgeableMob getBreedOffspring(ServerLevel world, AgeableMob mate) {
        return MUG_MUG.create(world, EntitySpawnReason.BREEDING);
    }
}
