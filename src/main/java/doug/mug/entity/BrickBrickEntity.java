...
public class BrickBrickEntity extends AbstractHorse {

    public BrickBrickEntity(EntityType<? extends AbstractHorse> type, Level world) {
        super(type, world);
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!held.isEmpty()) {
            if (this.isFood(held)) {
                ...
                if (this.isBaby()) {
                    this.usePlayerItem(player, hand, held);
                    this.ageUp(BrickBrickEntity.getSpeedUpSecondsWhenFeeding(-i), true);
                    return InteractionResult.SUCCESS;
                }
        ...
        if (this.saddleNotEquipped()) {
            if (held.is(Blocks.CRACKED_STONE_BRICKS.asItem())) {
                if (!this.level().isClientSide()) {
                    held.consume(1, player);
    ...
    public boolean canMate(Animal other) {
        ...
        return other instanceof BrickBrickEntity && this.isInLove() && other.isInLove();
    }

    @Override
    public AgeableMob getBreedOffspring(ServerLevel world, AgeableMob entity) {
        return BRICK_BRICK.create(world, EntitySpawnReason.BREEDING);
    }
}
