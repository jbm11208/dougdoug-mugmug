...
public class CowCowEntity extends AbstractHorse {

    public CowCowEntity(EntityType<? extends AbstractHorse> type, Level world) {
        super(type, world);
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!held.isEmpty()) {
            if (this.isFood(held)) {
                ...
                if (this.isBaby()) {
                    this.usePlayerItem(player, hand, held);
                    this.ageUp(CowCowEntity.getSpeedUpSecondsWhenFeeding(-i), true);
                    return InteractionResult.SUCCESS;
                }
            ...
            if (!this.fullOfMilk && held.is(Items.MILK_BUCKET) && !this.isBaby()) {
                this.fullOfMilk = true;
                held.consume(1, player);
                player.playSound(SoundEvents.MUD_STEP, 1.0f, 1.0f);
                player.getInventory().placeItemBackInInventory(new ItemStack(Items.BUCKET));
                return InteractionResult.SUCCESS;
            }
            if (this.fullOfMilk && held.is(Items.BUCKET) && !this.isBaby()) {
        ...
        if (this.saddleNotEquipped()) {
            if (held.is(Blocks.OAK_LOG.asItem())) {
                if (!this.level().isClientSide()) {
                    held.consume(1, player);
    ...
    public AgeableMob getBreedOffspring(ServerLevel world, AgeableMob entity) {
        return COW_COW.create(world, EntitySpawnReason.BREEDING);
    }
}
